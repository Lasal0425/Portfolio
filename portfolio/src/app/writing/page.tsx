...
import { engineeringPosts, personalPosts, type Post } from "@/lib/content";
      ...
      {posts.map((post) => (
        <li key={post.slug} className="rounded-lg border border-[var(--color-border)] p-4">
          ...
          <p className="mt-1.5 text-xs text-[var(--color-fg-muted)]">
            {post.source} · {new Date(post.publishedAt).toLocaleDateString("en-US", {
              year: "numeric",
              month: "short",
              day: "numeric",
            })}
 ...
 * A third post (or the RAG pipeline write-up from Phase 6) is one entry in
 * content/posts.ts away from appearing here.
 */
